...
필요한 로그인은 사용자가 직접 합니다. CAPTCHA, 요청 제한, 플랫폼의 접근 거부가 발생하면 해당 회차에서 그 사이트의 읽기를 멈추고 알립니다. 실제 공고를 가져올 수 없으면 탐색 방향 제안임을 명확히 표시합니다. 공고 탐색은 읽기 전용이며 메시지 전송, 온라인 프로필 수정, 자동 지원을 하지 않습니다.
