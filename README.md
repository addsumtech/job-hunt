...
| **apply · 制作简历与申请材料** | 确定目标，需要一套有针对性的材料 | 中英文定制简历（Word/PDF）、可选求职信、评审记录和面试准备提纲 |
...
你可以提供已有简历，也可以从教育、工作和项目经历开始，让 Agent 整理成简历初稿，再针对目标岗位调整重点。支持中文、英文及其他已支持语言，交付可编辑 Word 和排版后的 PDF。

完整求职报告会逐条核对最终保留岗位的完整职位描述；确实无法读取的岗位，会标出原因和待核实状态。列表摘要可以用于初筛，但不能因为已凑够岗位数量，就当作完整查询结果交付。需要先看初步清单时，你可以明确提出。
...
下图是从 PDF 渲染的虚构简历示例，姓名、学校、公司、项目和数字均用于演示。英文版见 [English README](README_EN.md#cv-and-report-examples)。

<p align="center">
  <a href="docs/assets/examples/cv-zh.png"><img src="docs/assets/examples/cv-zh.png" width="680" alt="虚构中文简历：教育背景、工作与实习经历、项目和技能"></a>
</p>

### 求职报告节选

以下为 2026 年 9 月 11 日真实岗位查询的脱敏报告节选，展示投递顺序、匹配依据、经验缺口和准备建议。这是保留原貌的历史样例，包含仅有摘要的线索；新版完整交付要求已补齐逐条详情核对。虚构简历的经历没有用于报告判断。点击图片可查看原尺寸。
