...
**지원 지역의 규칙과 출력 언어는 별도로 처리합니다.** 이력서 렌더러에는 영어, 네덜란드어, 독일어, 프랑스어, 스페인어, 이탈리아어, 중국어, 일본어, 한국어의 제목과 개인정보 항목명이 포함되어 있습니다. 현재 `discover`와 `assess`의 일부 필수 문구와 집계 카드는 영어·중국어만 지원합니다. 일본어, 한국어, 스페인어만으로 작성한 보고서는 검증에 실패하여 영어 또는 중국어 구조가 필요할 수 있습니다. 다섯 언어의 README가 모든 흐름의 완전한 현지화를 뜻하지는 않습니다. PDF에는 출력 언어에 맞는 글꼴도 필요합니다.
