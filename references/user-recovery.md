...
the owner, list every round this consultation delivers (normally the original
round and the new one) in `collection.yaml` and in `plan.rounds` of
`search-coverage.yaml`, then run
