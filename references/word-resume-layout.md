...
English body text is left-aligned so justification cannot stretch word spacing.
...
in `experience`; a visual reference does not introduce a new profile field or
require every candidate to use the example's section order. Photos are absent
